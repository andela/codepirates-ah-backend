...
describe('Test article tags', () => {
  ...
  describe('Tag an article', () => {
    it('should tag an article with an existing sinle tag', (done) => {
      chai.request(server)
    ...
    it('should not duplicate tagging', (done) => {
      chai.request(server)
        .post('/api/v1/articles/1/tags')
        .set('authorization', token)
        .send({ name: 'tag2' })
    ...
    it('should support mulriple simulatneous tagging', (done) => {
      chai.request(server)
        .post('/api/v1/articles/1/tags')
        .set('authorization', token)
  ...
  describe('Delete tags and associations', () => {
    it('Should delete a particular tag from a specific article', (done) => {
      chai.request(server)
        .delete('/api/v1/articles/4/2')
        .set('Authorization', token)
        .end((err, res) => {
          if (err) { done(err); }
          expect(res.status).to.be.equal(200);
          expect(res.body.message).to.include('tag with Id 2 removed');
          done();
        });
    });
    ...
    it('Should return appropriate respone if no articles found', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          if (err) { done(err); }
          expect(res.status).to.be.equal(404);
          expect(res.body.message).to.be.equal('no articles about tag11');
          done();
        });
    });
